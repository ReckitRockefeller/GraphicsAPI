...
namespace MadGrap
{
	public class ShapeEllipse: ColouredShape {
		public override void InternalDraw(Graphics g) {
			g.DrawEllipse(pen,x+penWidth,y+penWidth,w,h);
			g.FillEllipse(brush,x+penWidth+0.5F,y+penWidth+0.5F,w-1.0F,h-1.0F);
		}
		...
		public override bool OnPoint(int x, int y) {
			if ((x -= this.x) >= 0 && (y -= this.y) >= 0 && x <= bounds.Width && y <= bounds.Height) {
				...
				if (pen.Color.A != 0) {
					g.DrawEllipse(new Pen(Color.Red,(int)pen.Width),penWidth-x,penWidth-y,w,h);
				}
				if (brush.Color.A != 0) {
					g.FillEllipse(new SolidBrush(Color.Red),penWidth-x+0.5F,penWidth-y+0.5F,w-1.0F,h-1.0F);
				}
		...
		public ShapeEllipse(Color color, Rectangle rectangle):
			this(color,rectangle.X,rectangle.Y,rectangle.Width,rectangle.Height) {
		}

		public ShapeEllipse(Color color, Point position, Size size):
			this(color,position.X,position.Y,size.Width,size.Height) {
		}

		public ShapeEllipse(Color color, int x, int y, int w, int h) {
			this.x = x;
			this.y = y;
			this.w = w;
			this.h = h;
